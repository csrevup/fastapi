    ...
    try:
        with psycopg2.connect(connection_string_bonaparte) as conn:
            with conn.cursor() as cur:
                ...
                if not result["skus"]:
                    # If no items meet the threshold, take the top 2 if similarity >10%, if not return no matches
...
def get_image_url(image_name):
    ...
    return {"image_name":image_name,"image_url":s3.generate_presigned_url('get_object', Params={'Bucket': 'bonaparte-items-pictures', 'Key': image_name}, ExpiresIn=3600)}
